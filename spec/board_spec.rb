...
describe Board do
  ...
  describe '#match_four_of_a_kind' do
    before do
      ...
      context 'when there is four in a row vertically' do
        it 'returns true' do
          board.instance_variable_set(:@board,
                                      [['X', 1, 2, 3], ['X', 3, 2, 1], ['X', 2, 3, 1], ['X', 4, 5, 6], ['X', 6, 5, 4]])
    ...
    describe '#four_in_a_diagonal' do
      context 'when all cells are empty' do
        it 'returns false' do
          board.instance_variable_set(:@board, Array.new(6, Array.new(7)))
          expect(board.four_in_a_diagonal).to eq(false)
        end
      end
      ...
      context 'when there is four in a row diagonally' do
        it 'returns true' do
          ...
          expect(board.four_in_a_diagonal).to eq(true)
        end
      end
